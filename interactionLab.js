...
const InteractionLab = (function() {
    ...
    const DEFAULTS = {
        ...
        // PerformanceObserver('resource') + attach
        enableConsoleAdapter: false,
        // optional: capture console.* calls (noisy)

        // Noise control
        logResourceEntriesToConsole: false,
        maxRecentResources: 250,
        resourceMatchWindowMs: 250,
        // how close startTime must be to match fetch/xhr → resource entry
    ...
    function emit(level, tag, message, details, interactionId) {
        ...
        try {
            if (details !== undefined)
                fn.call(console, prefix, message, details);
            else
    ...
    function findSessionForTime(ts) {
        ...
        if (state.lastSessionId) {
            ...
            if (s && ts >= s.startTime && ts <= s.startTime + CONFIG.interactionWindowMs)
                return s;
        }
    ...
    function startInteractionFromEvent(ev) {
        ...
        console.groupCollapsed(`%c[InteractionStart]%c ${ev.type} on ${targetInfo.selector}`, 'color:#4caf50;font-weight:bold;', 'color:inherit;');
        ...
        if (state.observers.intersection && session._targetNode) {
            try {
                state.observers.intersection.observe(session._targetNode);
            } catch (_) {}
        }
        if (state.observers.resize && session._targetNode) {
            try {
                state.observers.resize.observe(session._targetNode);
            } catch (_) {}
        }
    ...
    function closeInteraction(id) {
        ...
        console.groupCollapsed(`%c[InteractionSummary]%c ${session.domEvent.type} on ${session.domEvent.target.selector}  ` + `→ INP≈${inp}ms, LoAF max ${loaf}ms, CLS ${cls.toFixed(3)}, net ${netCount}, LCPs ${lcpCount}`, 'color:#2196f3;font-weight:bold;', 'color:inherit;');
    ...
    function setupPerformanceObservers() {
        ...
        if (CONFIG.enableResourceTimings) {
            state.observers.resource = observeType('resource', function(entry) {
                if ((entry.duration || 0) < CONFIG.minResourceDurationMs)
                    return;
        ...
        try {
            if (performance && typeof performance.getEntriesByType === 'function') {
                ...
                if (nav && nav.length)
                    emit('info', 'NavigationTiming', 'snapshot', nav[0], null);
            }
        } catch (_) {}
    }
    ...
    function matchResourceTiming(url, startTime) {
        ...
        for (let i = state.recentResources.length - 1; i >= 0; i--) {
            ...
            if (bestDelta <= CONFIG.resourceMatchWindowMs && delta > bestDelta + CONFIG.resourceMatchWindowMs)
                break;
        }

        if (best && bestDelta <= CONFIG.resourceMatchWindowMs)
            return best;
        return null;
    }
    ...
    function setupNetworkHooks() {
        ...
        if (CONFIG.enableNetworkHooks && window.XMLHttpRequest && window.XMLHttpRequest.prototype) {
            ...
            proto.send = function patchedSend(body) {
                ...
                xhr.addEventListener('error', function(e) {
                    finalize('xhr-error', e || new Error('XHR error'));
                }, {
                    once: true
                });
                xhr.addEventListener('abort', function() {
                    finalize('xhr-abort', new Error('XHR abort'));
                }, {
                    once: true
                });
                xhr.addEventListener('timeout', function() {
                    finalize('xhr-timeout', new Error('XHR timeout'));
                }, {
                    once: true
                });
        ...
        if (CONFIG.enableNetworkHooks && typeof window.WebSocket === 'function') {
            ...
            window.WebSocket = function PatchedWebSocket(url, protocols) {
                const ws = protocols !== undefined ? new state.originals.WebSocket(url,protocols) : new state.originals.WebSocket(url);
    ...
    function setupErrorHooks() {
        ...
        const onError = function(msg, src, line, col, err) {
            ...
            if (session)
                attachToSession(session, 'errors', payload);
    ...
    function setupNavHooks() {
        ...
        function logNav(kind, extra) {
            ...
            if (session)
                attachToSession(session, 'nav', payload);
    ...
    function stop() {
        ...
        Object.keys(state.observers).forEach(function(key) {
            ...
            try {
                if (typeof obs.disconnect === 'function')
                    obs.disconnect();
                if (typeof obs.takeRecords === 'function')
                    obs.takeRecords();
            } catch (_) {}
        ...
        try {
            ...
            if (state.originals.sendBeacon)
                navigator.sendBeacon = state.originals.sendBeacon;
            ...
            if (state.originals.pushState)
                history.pushState = state.originals.pushState;
            if (state.originals.replaceState)
                history.replaceState = state.originals.replaceState;
    ...
    function getState() {
        return {
            ...
            observers: Object.keys(state.observers).filter(function(k) {
                return !!state.observers[k];
            }),
